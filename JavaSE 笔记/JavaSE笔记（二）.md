...
传入方法的参数，如果是**基本类型，会在调用方法的时候，对参数的值进行复制**，方法中的参数变量，不是我们传入的变量本身！
...
**静态变量和静态方法**是类具有的属性（后面还会提到静态类、静态代码块），也可以理解为是**所有对象共享的内容**。我们通过使用**`static`**关键字来声明一个变量或一个方法为静态的，一旦被声明为静态，那么通过这个类创建的所有对象，操作的都是同一个目标，也就是说，对象再多，也只有这一个静态的变量或方法。那么，一个对象改变了静态变量的值，那么其他的对象读取的就是被改变的值。
...
所有**被标记为静态的内容，会在类刚加载的时候就分配**，而不是在对象创建的时候分配，所以说静态内容一定会在第一个对象初始化<u>之前完成加载。</u>
...
代码块<u>在对象创建时执行</u>，也是属于类的内容，**但是它在构造方法执行之前执行**（和成员变量初始值一样），<u>且每创建一个对象时，只执行一次！（相当于构造之前的准备工作）</u>
...
![image-20220310010215098](/Users/wsp/Library/Application Support/typora-user-images/image-20220310010215098.png)
...
数组做实参，因为数组也是类，所以**形参得到的是数组的引用**而不是复制的数组，**操作的依然是数组对象本身**
...
foreach属于增强型的for循环，它使得代码更简洁，同时我们能**直接拿到数组中的每一个数字。** 
...
多态是**同一个行为具有多个不同表现形式或形态的能力**。也就是同样的方法，由于实现类不同，执行的结果也不同！

#### 方法的重写

我们之前学习了方法的重载，方法的重写和重载是不一样的，重载是原有的方法逻辑不变的情况下，支持更多参数的实现，而**重写是直接覆盖原有方法！**
...
重写是为了运行时多态，而**静态方法初始化时就和类做了绑定**，**没有多态特征**，所以静态方法不能被重写，也没必要重写。
...
**同理，如果想访问父类的成员变量，也可以使用super关键字来访问**，注意，子**类可以具有和父类相同的成员变量**！而在方法中访问的默认是 形参列表中 > 当前类的成员变量 > 父类成员变量
...
我们曾经学习过基本数据类型的类型转换，支持一种数据类型转换为另一种数据类型，而我们的**类也是支持类型转换**的（**仅限于存在亲缘关系的类**之间进行转换）比如子类可以直接**向上转型**：
...
我们也可以**把已经明确是由哪个类实现的父类引用**，**强制转换为对应的类型**：

```java
Student student = new SportStudent("lbw", 20);  //是由SportsStudent进行实现的
//... do something...

SportStudent ps = (SportStudent)student;  //让它变成一个具体的子类
ps.sport();  //调用具体实现类的方法
```
...
如果**类的成员属性被声明为final，那么必须在构造方法中或是在定义时赋初始值**！
...
类本身就是一种抽象，而抽象类，把类还要抽象，也就是说，**抽象类可以只保留特征，而不保留具体呈现形态**，比如方法可以定义好，但是我可以不去实现它，而是**交由子类来进行实现**！
...
因此，抽象类一般**只用作继承使用**！抽象类使得继承关系之间更加明确：
...
接口甚至比抽象类还抽象，他只代表某个确切的功能！也就是**只包含方法的定义**，甚至都不是一个类！接口包含了一些列方法的具体定义，<u>类可以实现这个接口，表示类支持接口代表的功能</u>（类似于一个插件，只能作为一个附属功能加在主体上，同时具体实现还需要由主体来实现）
...
接口中定义的变量，默认为**public static final**
...
一个类可以实现很多个接口，但是不能理解为多继承！（实际上**实现接口是附加功能**，和继承的概念有一定出入，顶多说是多继承的一种替代方案）一个类可以附加很多个功能！
...
**类通过`implements`关键字来声明实现的接口**！每个接口之间用<u>逗号</u>隔开！
...
匿名内部类才是我们的重点，也是实现lambda表达式的原理！匿名内部类其实就是**在new的时候，直接对接口或是抽象类的实现：**
...
我们不用单独去创建一个类来实现，而是可以**直接在new的时候写对应的实现**！但是，这样写，**无法实现复用，只能在这里使用！**
